...
router.get("/records/appointment/:appointmentId", async function(req, res) {
  try {
    const doctorId = req.auth?.user?.doctorId;
    if (!doctorId) {
      return res.status(403).json({ ok: false, message: "Không tìm thấy hồ sơ bác sĩ" });
    ...
    const records = await listDoctorRecords(doctorId);
    const record = records.find((r) => r.appointmentId === appointmentId);
    if (!record) {
      return res.status(404).json({ ok: false, message: "Không tìm thấy hồ sơ bệnh án cho cuộc hẹn này" });
    }

    res.json({ ok: true, record });
  } catch (error) {
    console.error("[ROUTES] GET /doctor/records/appointment/:appointmentId ERROR:", error.message);
    res.status(500).json({ ok: false, message: error.message });
  }
});
