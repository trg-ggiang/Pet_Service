...
router.put("/profile", async function putProfile(req, res) {
  try {
    const profile = await updateStaffProfile(req.auth?.user?.staffId, req.body ?? {});
    res.json({ ok: true, profile });
  } catch (error) {
    sendError(res, error);
  }
...
router.put("/password", async function putPassword(req, res) {
  try {
    await updateStaffPassword(req.auth?.user?.id, req.body?.oldPassword, req.body?.newPassword);
