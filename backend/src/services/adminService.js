...
async function readStaffsTable() {
  // Try querying with department & position first; fall back to basic columns if they don't exist yet.
  const { data, error } = await supabase
    .from("staffs")
    .select("id, user_id, full_name, phone, address, department, position");
  if (!error) return data || [];

  // If the error is about missing columns, retry with basic columns only
  if (error.code === "42703" || (error.message && error.message.includes("does not exist"))) {
    console.warn("[adminService] staffs.department/position columns missing; falling back to basic select");
    const fallback = await supabase
      .from("staffs")
      .select("id, user_id, full_name, phone, address");
    if (fallback.error) throw new Error(fallback.error.message);
    return (fallback.data || []).map((row) => ({ ...row, department: null, position: null }));
  }
...
async function listAdminStaff(filters = {}) {
  ...
  const staff = users.staff.map((member) => ({
    ...
    department: member.department || "reception",
...
async function getAdminDashboard() {
  ...
  const activeBoardingByCage = new Map(
    boardings
      .filter((boarding) => ["BOOKED", "CHECKED_IN", "STAYING"].includes(String(boarding.current_status || "")))
...
async function updateAdminSettings(input, authUser) {
  if (!authUser?.id) {
    throw new Error("Thiếu thông tin người dùng xác thực");
  }

  // 1. Sync Email to users table if email changed
  if (input?.account?.email && input.account.email !== authUser.email) {
    const nextEmail = String(input.account.email).trim().toLowerCase();
    ...
    if (emailError) throw new Error("Không thể cập nhật email đăng nhập: " + emailError.message);
  }

  // 2. Sync Password to users table if requested
  if (input?.account?.newPassword) {
    const { oldPassword, newPassword } = input.account;
    if (!oldPassword) {
      throw new Error("Vui lòng cung cấp mật khẩu hiện tại để đổi mật khẩu.");
    }
    if (newPassword.length < 6) {
      throw new Error("Mật khẩu mới phải có tối thiểu 6 ký tự.");
    }

    // Fetch user hash & auth_version
    ...
    if (passError) throw new Error("Không thể cập nhật mật khẩu: " + passError.message);
  }

  // 3. Clean sensitive password fields before saving settings
  const cleanedAccount = input.account ? {
    name: input.account.name,
    email: input.account.email,
    phone: input.account.phone,
    role: input.account.role,
    initials: input.account.initials,
  } : {};
  ...
  const next = {
    ...
    account: { ...current.account, ...cleanedAccount },
