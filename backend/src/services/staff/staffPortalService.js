...
async function getStaffProfile(staffId) {
  ...
  const { data, error } = await supabase
    .from("staffs")
    .select("id, full_name, phone, address, department, position, users:user_id (email)")
  ...
  return {
    ...
    initials: data.full_name
      ...
      .join("")
      .toUpperCase(),
    roleLabel: data.position || deptLabels[data.department] || "Nhân viên chăm sóc",
...
async function updateStaffProfile(staffId, input) {
  const effectiveStaffId = assertStaffId(staffId);
...
async function updateStaffPassword(userId, oldPassword, newPassword) {
