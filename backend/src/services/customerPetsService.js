...
async function getCustomerPetDashboard(customerId) {
  ...
  return {
    customer: {
      ...
      tier: appointments.length >= 10 ? "vip" : "regular",
    },
    species,
    breeds,
    pets: pets.map((pet) =>
...
async function getCustomerProfile(customerId) {
  ...
  const { data, error } = await supabase
    .from("customers")
    .select("id, full_name, phone, address, user_id, users:user_id (email)")
    .eq("id", effectiveCustomerId)
    .single();
  if (error || !data) throw new Error("Không tìm thấy thông tin khách hàng");
...
async function updateCustomerPassword(userId, oldPassword, newPassword) {
