...
function bucketLabel(date, period, appointment) {
  if (period === "quarter") return `T${date.getMonth() + 1}`;
  if (period === "month") return `${date.getDate()}/${date.getMonth() + 1}`;
  if (period === "day") {
    if (appointment && appointment.requested_time) {
      return String(appointment.requested_time).slice(0, 5);
    }
    return formatDateShort(date).slice(0, 5);
  }
...
function buildStatsForPeriod(period, appointments, reviews) {
  ...
  rows.sort((a, b) => {
    ...
    const timeA = a.requested_time ? String(a.requested_time) : "";
    const timeB = b.requested_time ? String(b.requested_time) : "";
  ...
  rows.forEach((appointment) => {
    const date = new Date(getAppointmentDate(appointment));
    const label = bucketLabel(date, period, appointment);
  ...
  return {
    ...
    trend: [...buckets.values()].reverse(),
    byDay: [...buckets.values()].reverse().map((row) => ({ label: row.label, value: row.total })),
