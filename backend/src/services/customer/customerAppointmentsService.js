...
async function listAvailableCages(checkInDateStr, checkOutDateStr) {
  ...
  // 1. Get all cages that are marked as AVAILABLE or CLEANING (not MAINTENANCE)
  const { data: allCages, error: cagesError } = await supabase
    .from("cages")
    .select("id, cage_number, status, note")
    .neq("status", "MAINTENANCE");

  if (cagesError) throw new Error(cagesError.message);

  // 2. Get active boardings that overlap with the selected dates
  const { data: activeBoardings, error: boardingsError } = await supabase
    .from("boarding")
    .select("id, cage_id, check_in, check_out, current_status")
    .not("current_status", "in", "(CANCELLED,CHECKED_OUT)");

  if (boardingsError) throw new Error(boardingsError.message);

  const occupiedCageIds = new Set();
  const reqStart = checkIn.getTime();
  const reqEnd = checkOut.getTime();

  for (const boarding of activeBoardings || []) {
    const bStart = new Date(boarding.check_in).getTime();
    const bEnd = new Date(boarding.check_out).getTime();
    if (bStart < reqEnd && bEnd > reqStart) {
      occupiedCageIds.add(boarding.cage_id);
    }
  }

  return (allCages || []).filter((cage) => !occupiedCageIds.has(cage.id));
}
...
async function createCustomerAppointment(input, customerId) {
  ...
  let boardingPayload = null;
  if (insertPayload.appointment_type === "BOARDING") {
    const cageId = Number(input?.cageId || input?.cage_id);
    const checkIn = input?.checkIn || input?.check_in || requestedDate;
    const checkOut = input?.checkOut || input?.check_out;

    if (!cageId || !checkOut) {
      const error = new Error("Vui lòng chọn chuồng/phòng và ngày trả phòng khi đặt lưu trú.");
      error.statusCode = 400;
      throw error;
    }

    const availableCages = await listAvailableCages(checkIn, checkOut);
    const isAvailable = availableCages.some((c) => c.id === cageId);
    if (!isAvailable) {
      const error = new Error("Phòng/chuồng này đã có người đặt trong thời gian yêu cầu.");
      error.statusCode = 409;
      throw error;
    }
    ...
    boardingPayload = {
      ...
      current_status: "BOARDING_STATUS_BOOKED", // matches BOOKED enum/value
    };
    // Let's check BoardingStatus enum values from schema.prisma:
    // BOOKED, CHECKED_IN, STAYING, CHECKED_OUT, CANCELLED.
    // So current_status is "BOOKED". Let's use string "BOOKED".
    boardingPayload.current_status = "BOOKED";
    boardingPayload.special_note = input?.note?.trim() || null;
  }
  ...
  if (insertPayload.appointment_type === "BOARDING" && boardingPayload) {
    ...
    if (boardingError) {
      ...
      throw new Error("Không thể tạo thông tin lưu trú: " + boardingError.message);
    }
  }
