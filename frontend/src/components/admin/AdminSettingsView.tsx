  ...
  async function saveLocal() {
    ...
    if (oldPassword || newPassword || confirmPassword) {
      ...
      if (newPassword !== confirmPassword) {
        setLocalError("Mật khẩu mới và mật khẩu xác nhận không khớp.");
        return;
      }
      payload.account = {
        ...payload.account,
    ...
    try {
      ...
      setLocalError(err instanceof Error ? err.message : "Không thể lưu cài đặt.");
    }
  }
    ...
    if (tab === "account") {
      return (
        <div>
          ...
          <Field label="Mật khẩu hiện tại" description="Nhập mật khẩu hiện tại nếu bạn muốn thay đổi mật khẩu">
            <div className="relative">
              <input
                type={showPassword ? "text" : "password"}
                value={oldPassword}
                onChange={(e) => { setOldPassword(e.target.value); setLocalError(""); }}
                className="h-9 w-64 rounded-xl border border-border bg-white px-3 pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/20"
              />
          ...
          <Field label="Mật khẩu mới">
            <input
              ...
              className="h-9 w-64 rounded-xl border border-border bg-white px-3 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/20"
            />
          </Field>
          ...
          {localError && (
            <div className="mt-4 p-3 rounded-xl border border-amber-200 bg-amber-50 text-xs font-semibold text-amber-700">
              {localError}
            </div>
          )}
          <SaveButton saved={saved} onClick={saveLocal} />
