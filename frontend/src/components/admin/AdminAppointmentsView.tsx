...
interface Appointment {
  ...
  species: string; service: string; staff: string;
  status: Status; amount: string; phone?: string; email?: string; notes?: string;
}

const EMPTY_SUMMARY: AdminAppointmentSummary = {
  total: 0,
  filtered: 0,
  totalAmount: 0,
  totalAmountText: "0",
  tabs: [
    { status: "all", label: "Tất cả", count: 0 },
    { status: "scheduled", label: "Chờ khám", count: 0 },
    { status: "in_progress", label: "Đang khám", count: 0 },
    { status: "completed", label: "Hoàn thành", count: 0 },
    { status: "cancelled", label: "Đã huỷ", count: 0 },
  ],
  serviceOptions: [],
  staffOptions: [],
};
...
export function AdminAppointmentsView() {
  ...
  const [summary, setSummary] = useState<AdminAppointmentSummary>(EMPTY_SUMMARY);
  ...
  useEffect(() => {
    ...
    adminService
      .listAppointments({ status: statusFilter, search })
      .then((data) => {
        if (!mounted) return;
  ...
  async function handleStatusChange(id: string, status: Status) {
    try {
      setActionError("");
      const data = await adminService.updateAppointmentStatus(id, status);
      setApts((prev) => prev.map((a) => a.id === id ? data.appointment as Appointment : a));
      if (drawerApt?.id === id) setDrawerApt(data.appointment as Appointment);
      const refreshed = await adminService.listAppointments({ status: statusFilter, search });
      setApts(refreshed.appointments as Appointment[]);
      setSummary(refreshed.summary);
    } catch (error) {
      setActionError(error instanceof Error ? error.message : "Không thể cập nhật trạng thái lịch hẹn.");
    }
  }
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-1 bg-card border border-border p-1 rounded-lg">
          {summary.tabs.map(({ status: val, label, count: cnt }) => {
            return (
              <button
                key={val}
