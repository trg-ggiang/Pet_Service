...
export function CustomerHomeTab({
  ...
  unreadCount: number;
  customerInfo: { tier: "regular" | "vip"; totalVisits: number } | null;
                ...
                <div className="relative z-0 col-span-6 md:col-span-4 md:col-start-1 md:row-start-1 row-span-3 rounded-3xl p-6 text-white shadow-md overflow-hidden" style={{ background: "linear-gradient(135deg,#0891B2 0%,#06B6D4 100%)" }}>
                  <div className="relative z-0 h-full flex flex-col">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {customerInfo?.tier === "vip" && (
                        <span className="px-2 py-0.5 rounded-full bg-amber-400 text-[10px] font-bold text-slate-950 uppercase tracking-wider flex items-center gap-0.5 shadow-sm border border-amber-300 leading-none">
                          ⭐ VIP
                        </span>
                      )}
                    </div>
                    ...
                    <div className="mt-4 flex flex-wrap items-center gap-3">
                      <div className="inline-flex items-center gap-2 bg-white/20 backdrop-blur-md px-3 py-1.5 rounded-lg text-sm font-medium w-fit">
                        <Calendar size={14} />
                        <span>Có <strong>{apts.length} lịch hẹn</strong> sắp tới</span>
                      </div>
                      
                      {customerInfo && customerInfo.tier === "vip" && (
                        <div className="inline-flex items-center gap-1.5 bg-amber-500/25 border border-amber-400/30 backdrop-blur-md px-3 py-1.5 rounded-lg text-xs font-semibold text-amber-200">
                          👑 VIP Member
                        </div>
                      )}
                    </div>
                    ...
                    {customerInfo && (
                      <div className="mt-3.5 max-w-[280px]">
                        {customerInfo.tier === "vip" ? (
                          <p className="text-[11px] text-amber-200/90 font-medium">👑 Cảm ơn bạn đã là thành viên VIP thân thiết của PetCare! Bạn đã thực hiện tổng cộng {customerInfo.totalVisits} lượt đặt lịch khám.</p>
                        ) : (
                          <div>
                            <div className="flex justify-between text-[11px] font-medium opacity-90 mb-1">
                              <span>Tiến trình nâng cấp VIP</span>
                              <span>{customerInfo.totalVisits} / 10 lịch hẹn</span>
                            </div>
                            <div className="w-full h-1.5 bg-white/20 rounded-full overflow-hidden">
                              <div
                                className="h-full bg-amber-400 rounded-full transition-all duration-500"
                                style={{ width: `${Math.min(100, (customerInfo.totalVisits / 10) * 100)}%` }}
                              />
                            </div>
                            <p className="text-[10px] opacity-80 mt-1.5 font-medium">Đặt thêm {10 - customerInfo.totalVisits} lịch hẹn nữa để trở thành VIP nhé!</p>
                          </div>
                        )}
                      </div>
                    )}
