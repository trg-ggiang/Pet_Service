...
import {
  ...
  ImagePlus, Loader2, Play, RefreshCw, Search, X,
} from "lucide-react";
...
interface QueueItem {
  ...
  note: string | null;
  ordered_at: string;
  is_mine_to_perform: boolean;
  ordered_by_doctor_name: string | null;
  performed_by_doctor_name: string | null;
...
export function DoctorSpecialistQueueTab() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const data = await apiFetch<{ ok: boolean; queue: QueueItem[] }>(
        `/api/doctor/appointments/specialist-queue?${params.toString()}`,
      );
      setQueue(data.queue ?? []);
    } catch (err) {
  ...
  async function handleStart(item: QueueItem) {
    setActingId(item.id);
    try {
      await apiFetch(`/api/doctor/appointments/service-orders/${item.id}/start`, { method: "PATCH" });
      await load();
    } catch (err) {
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Tìm tên/SĐT chủ nuôi..."
            className="h-9 w-56 rounded-xl border border-slate-200 bg-white pl-8 pr-3 text-sm text-slate-700 placeholder:text-slate-400 focus:outline-none focus:border-cyan-400"
          />
        </div>
        ...
        <div className="relative">
          ...
          <select
            ...
            className="h-9 appearance-none rounded-xl border border-slate-200 bg-white pl-8 pr-8 text-sm font-semibold text-slate-700 focus:outline-none focus:border-cyan-400"
          >
        ...
        {!loading && queue.length > 0 && (
          <div className="flex items-center gap-2 text-sm text-slate-500">
            ...
            <span>·</span>
            <span className="font-semibold text-emerald-600">{doneCount} có kết quả</span>
          </div>
        )}
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center gap-3 rounded-xl border border-dashed border-slate-200 bg-white py-16 text-center">
          ...
          <div>
            <p className="text-sm font-bold text-slate-700">Không có chỉ định nào</p>
            <p className="mt-1 text-xs text-slate-400">
              {search ? `Không tìm thấy kết quả cho "${search}"` : "Chưa có dịch vụ chuyên sâu nào được chỉ định"}
            </p>
          </div>
        </div>
      ) : (
                ...
                <div className="px-4 py-3.5 flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start gap-3">
                      <div className="flex-1">
                        ...
                        <p className="mt-0.5 text-[11px] text-slate-400">
                          BS chỉ định: <span className="font-medium text-slate-600">{item.ordered_by_doctor_name ?? "Không rõ"}</span>
                          {item.performed_by_doctor_name && (
                            <span className="ml-2 text-emerald-600">
                              · BS thực hiện: <span className="font-medium">{item.performed_by_doctor_name}</span>
                            </span>
                          )}
                        </p>
                      </div>
                    ...
                    <div className="mt-2 flex items-center gap-2 flex-wrap">
                      ...
                      <span className="text-[11px] bg-cyan-50 text-cyan-700 border border-cyan-100 rounded-md px-2 py-0.5 font-semibold">
                        {ROOM_OPTIONS.find(o => o.value === item.services.specialist_room_type)?.label ?? item.services.specialist_room_type}
                      </span>
                    ...
                    {/* Specialist notes */}
                    {item.status === "COMPLETED" && item.service_order_results?.notes && (
                      <p className="mt-1.5 text-[11.5px] text-slate-500 italic">
                        Nhận xét: {item.service_order_results.notes}
                      </p>
                    )}
                  </div>
                  ...
                  <div className="flex flex-col gap-2 flex-shrink-0">
                    ...
                    {item.status === "IN_PROGRESS" && item.is_mine_to_perform && (
                      <button
                    ...
                    {item.status === "IN_PROGRESS" && !item.is_mine_to_perform && (
                      <span className="flex items-center gap-1.5 h-8 px-3 rounded-lg bg-slate-100 text-slate-400 text-xs font-semibold cursor-not-allowed">
...
function ResultModal({
  ...
  async function handleSubmit() {
    if (!resultText.trim() && imageUrls.length === 0) {
      setError("Cần nhập kết quả hoặc thêm hình ảnh");
      return;
    }
    setSubmitting(true);
    setError("");
    try {
      await apiFetch(`/api/doctor/appointments/service-orders/${item.id}/result`, {
        method: "POST",
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4" onClick={onClose}>
      <div className="w-full max-w-[520px] rounded-2xl border border-slate-200 bg-white shadow-2xl" onClick={e => e.stopPropagation()}>
        {/* Header */}
        <div className="flex items-start justify-between px-5 py-4 border-b border-slate-100">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">
              {isReadOnly ? "Kết quả dịch vụ" : "Nhập kết quả"}
            </p>
        ...
        <div className="px-5 py-4 flex flex-col gap-3.5">
          {/* Result text */}
          <div>
            <label className="text-[11px] font-bold uppercase tracking-wider text-slate-400 block mb-1.5">Mô tả kết quả / nhận xét</label>
            <textarea
              value={resultText}
              onChange={e => setResultText(e.target.value)}
              readOnly={isReadOnly}
              rows={4}
              placeholder="Nhập nhận xét, chỉ số, kết luận chuyên sâu..."
          ...
          <div>
            <label className="text-[11px] font-bold uppercase tracking-wider text-slate-400 block mb-1.5">
              Nhận xét chuyên sâu của bác sĩ
            </label>
            {isReadOnly ? (
              <p className="text-[13px] text-slate-700 bg-slate-50 rounded-xl px-3.5 py-2.5 border border-slate-100 min-h-[40px]">
                {existing?.notes || <span className="text-slate-400 italic">Không có ghi chú</span>}
              </p>
            ) : (
              <input
                value={notes}
                onChange={e => setNotes(e.target.value)}
                placeholder="Nhận xét, gợi ý điều trị, khuyến cáo tiếp theo..."
          ...
          <div>
            ...
            {imageUrls.length > 0 ? (
                ...
                {imageUrls.map((url, i) => (
                  <div key={i} className="relative group">
                    <a href={url} target="_blank" rel="noreferrer">
                      <img src={url} alt="" className="h-16 w-16 rounded-lg object-cover border border-slate-200 hover:opacity-80 transition-opacity" />
                    </a>
                    {!isReadOnly && (
                      <button
              ...
              <p className="text-[12px] text-slate-400 italic">Không có hình ảnh</p>
            )}
          </div>
