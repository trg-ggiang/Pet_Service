...
export interface AvailableCage {
  id: number;
  cage_number: string;
  status: string;
  note: string | null;
...
export async function fetchAvailableCages(checkIn: string, checkOut: string): Promise<AvailableCage[]> {
  const query = new URLSearchParams({ checkIn, checkOut });
  const payload = await requestJson<{ ok: true; cages: AvailableCage[] }>(
    `/api/customer/cages/available?${query.toString()}`,
    { headers: getAuthHeaders() },
  );
  return payload.cages;
}
...
export async function submitCustomerReview(input: {
  ...
  const payload = await requestJson<{ ok: true; review: any }>(
...
export async function fetchCustomerReview(appointmentId: number): Promise<any> {
  const payload = await requestJson<{ ok: true; review: any }>(
    `/api/customer/reviews/${appointmentId}`,
    { headers: getAuthHeaders() },
  );
  return payload.review;
}
