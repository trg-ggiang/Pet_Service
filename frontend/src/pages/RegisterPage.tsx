...
import { Eye, EyeOff, CheckCircle2, ChevronRight, ArrowLeft, Info, Heart, Stethoscope, Calendar } from "lucide-react";
...
export function RegisterPage({ onBack, onSuccess }: { onBack: () => void; onSuccess: () => void }) {
  ...
  function Field({
    ...
    return (
      <div>
        <label htmlFor={id} className="text-[11px] font-bold uppercase tracking-[0.07em] text-muted-foreground">{label}</label>
    ...
    <div className="min-h-screen flex bg-white">
      ...
      >
        ...
        <div
          ...
          style={{ width: 320, height: 320, background: "radial-gradient(circle, rgba(8,145,178,0.12) 0%, transparent 70%)" }}
        />
        ...
        <div className="relative z-10 flex flex-col h-full px-10 py-14">
          ...
          <div className="mt-auto mb-auto pt-16">
            ...
            <p className="text-slate-400 mt-3 text-sm leading-relaxed max-w-[270px]">
              Đăng ký để quản lý lịch hẹn và theo dõi sức khoẻ cho thú cưng của bạn.
            </p>
            ...
            <div className="mt-8 p-4 rounded-xl" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}>
              <div className="flex items-start gap-2.5">
                ...
                <p className="text-[12px] text-slate-500 leading-relaxed">
                  Tài khoản <span className="text-slate-400 font-semibold">bác sĩ & nhân viên</span> được cấp trực tiếp bởi quản trị viên — không qua đăng ký này.
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-10 bg-slate-50/50">
        <div className="w-full max-w-[400px]">
          ...
          <form onSubmit={handleSubmit} className="mt-7 space-y-4">
            <Field label="Họ và tên" id="name" value={form.name} onChange={set("name")} placeholder="Nguyễn Văn A" error={errors.name} />

            <div className="grid grid-cols-2 gap-3">
              <Field label="Email" id="email" type="email" value={form.email} onChange={set("email")} placeholder="email@example.com" error={errors.email} />
              <Field label="Số điện thoại" id="phone" value={form.phone} onChange={set("phone")} placeholder="09xxxxxxxx" error={errors.phone} />
            </div>
            ...
            <div>
              ...
              >
                ...
                <span className="text-sm text-muted-foreground leading-relaxed select-none">
                  Tôi đồng ý với{" "}
                  <button type="button" onClick={e => e.stopPropagation()} className="text-cyan-600 font-semibold hover:underline">
                    Điều khoản dịch vụ
                  </button>{" "}
                  và{" "}
                  <button type="button" onClick={e => e.stopPropagation()} className="text-cyan-600 font-semibold hover:underline">
            ...
            <p className="text-center text-sm text-muted-foreground">
              Đã có tài khoản?{" "}
              <button type="button" onClick={onBack} className="font-semibold text-cyan-600 hover:underline">
                Đăng nhập
              </button>
            </p>
          </form>
