...
import { Bell, Calendar, CheckCircle2, Clock, Heart, LogOut, Star, X, Settings } from "lucide-react";
...
import { CustomerSettingsTab } from "../../../components/customer/settings/CustomerSettingsTab";
import { getNotifConfig, mapCustomerAppointment, mapCustomerNotification } from "../../../utils/customer/portalConfig";

type CustomerTab = "home" | "apts" | "pets" | "history" | "notifications" | "settings";
...
export function CustomerPortal({ onLogout, userName }: { onLogout: () => void; userName: string }) {
  ...
  const [customerInfo, setCustomerInfo] = useState<{ tier: "regular" | "vip"; totalVisits: number } | null>(null);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (dash.customer) {
          setCustomerInfo({
            tier: dash.customer.tier ?? "regular",
            totalVisits: dash.customer.totalVisits ?? 0,
          });
        }
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-3 pl-3 border-l border-slate-200">
            <div className="w-10 h-10 rounded-full flex items-center justify-center shadow-sm" style={{ background: "linear-gradient(135deg,#0891B2,#06B6D4)" }}>
              <span className="text-sm font-bold text-white">{(currentUserName || "").split(/\s+/).filter(Boolean).slice(0, 2).map((part) => part[0]).join("").toUpperCase() || "KH"}</span>
            </div>
            <div className="hidden sm:block">
              <div className="flex items-center gap-1.5">
                <span className="text-sm font-semibold text-foreground">{currentUserName}</span>
                {customerInfo?.tier === "vip" && (
                  <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded bg-amber-500 text-[10px] font-bold text-white shadow-sm leading-none">
                    ⭐ VIP
                  </span>
                )}
              </div>
              <div className="text-[11px] font-medium text-cyan-600 bg-cyan-50 px-2 py-0.5 rounded-full inline-flex mt-0.5">
                {customerInfo?.tier === "vip" ? "Khách hàng VIP" : "Khách hàng"}
              </div>
            </div>
          </div>
          <button
