...
export const staffAppointmentsService = {
  ...
  async updateProfile(input: { fullName: string; phone: string; address: string }): Promise<StaffProfile> {
    const data = await fetchWithAuth<ProfileResponse>("/api/staff/profile", {
      method: "PUT",
  ...
  async updatePassword(input: { oldPassword: string; newPassword: string }): Promise<void> {
    await fetchWithAuth<MutationResponse>("/api/staff/password", {
      method: "PUT",
      body: JSON.stringify(input),
    });
  },
