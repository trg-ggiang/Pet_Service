...
export const doctorDataService = {
  ...
  async getRecordByAppointmentId(appointmentId: number): Promise<DoctorMedicalRecord> {
    const data = await requestJson<ApiOk<{ record: DoctorMedicalRecord }>>(
      apiUrl(`/api/doctor/records/appointment/${appointmentId}`),
      authInit(),
    );
    return data.record;
  },
